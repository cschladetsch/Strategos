#include <iostream>    // Standard headers first
#include <array>
#include <map>
#include <string>

#include "board.hpp"   // Local headers
#include <spdlog/spdlog.h>
#include "rang.hpp"    // Third-party headers last
    ...
    for (int y = 0; y < size; ++y) {
        ...
        for (int x = 0; x < size; ++x) {
            ...
            if (x == cursor_x && y == cursor_y) {
                ...
                attron(A_REVERSE);
                cell << grid[y][x];
                mvprintw(y + 1, 4 + x * 2, "%s", cell.str().c_str());
                attroff(A_REVERSE);
            } else if (grid[y][x] == "K" || grid[y][x] == "N" || grid[y][x] == "B" || grid[y][x] == "R") {
                // Apply `rang` colors for pieces
                if (player_turn == 1) {
                    cell << rang::fg::red << grid[y][x] << rang::style::reset;
                } else {
                    cell << rang::fg::blue << grid[y][x] << rang::style::reset;
                }
                mvprintw(y + 1, 4 + x * 2, "%s", cell.str().c_str());
            } else {
